...
class BasketTest(BaseTest):
    ...
    def test_add_a_product_to_basket(self):
        ...
        self.assertNotEqual(resp.json().get('products'), [])
    ...
    def test_remove_a_product_to_basket(self):
        ...
        self.assertEqual(self.user.basket.products.first().id , self.products[0].id)
        ...
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.json().get('products'), [])
